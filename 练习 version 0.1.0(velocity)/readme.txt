...
·为所有的类和方法添加注释
·新建Maven工程logic
·将ui中的dao、service、bll、beans、util包放在logic工程中
·复制logic工程的配置信息加入ui工程的pom.xml配置文件中，完成logic工程和ui工程的关联

version 0.0.9
-----------------------------------------------------------
·完成UUID的Junit测试，添加测试类UUIDTest.java
·完成Cookie的Junit测试，添加测试类CookieTest.java
·完成Memcache的Junit测试，添加测试类MemcacheTest.java
·加入Cookie，在用户登录验证通过后，创建Cookie，保存一个随机字符串UUID到Cookie中
·加入Memcache
	MemcachedClientBuilder builder = new XMemcachedClientBuilder("127.0.0.1:11211");
	MemcachedClient client = builder.build();
	client.set(uuid, 0, users);
	在用户访问主页是会拿Cookie的UUID和client.get(cookie.getValue())比较，相同会返回user，用户进入主页，不相同的话会跳转到登录页面

version 0.1.0
-----------------------------------------------------------
·添加了Velocity的配置，增加vm文件高亮插件
·修改jsp文件，删除jsp头部内容，并更名为vm后缀
·删除JSTL标签，使用Velocity代替JSTL标签
·修改登录控制UserLoginController
